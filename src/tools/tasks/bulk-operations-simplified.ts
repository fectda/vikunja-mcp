      ...
      const updateResult = await processors.update.processBatches(taskIds, async (taskId) => {
        ...
        if (args.field === 'assignees' && Array.isArray(args.value)) {
          const currentAssignees =
            (await client.tasks.getTask(taskId)).assignees?.map((a: Assignee) => a.id) || [];
    ...
    try {
      ...
      if (Array.isArray(result) && result.length > 0) {
        const isValid = result.every(
          (t) => t && typeof t === 'object' && 'project_id' in t && 'title' in t,
        );
        let hasValue = true;
      ...
      if (shouldFallback) {
        logger.warn(
          'Bulk update API returned success but values not updated, falling back to individual updates',
        );
        return await updateWithFallback();
      }

      const fetchResult = await processors.update.processBatches(
        taskIds,
        async (id) => await client.tasks.getTask(id),
      );
      return successResponse(
        'update-task',
        `Successfully updated ${taskIds.length} tasks${fetchResult.failed.length > 0 ? ` (${fetchResult.failed.length} could not be fetched)` : ''}`,
        fetchResult.successful,
        {
          count: taskIds.length,
          affectedFields: [args.field],
          ...(fetchResult.failed.length > 0 && { fetchErrors: fetchResult.failed.length }),
        },
      );
    } catch (bulkError) {
    ...
    const creationResult = await processors.create.processBatches(
      ...
      async (index) => {
        ...
        if (t.repeatAfter !== undefined || t.repeatMode !== undefined) {
          ...
          if (rc.repeat_mode !== undefined)
            (newTask as Record<string, unknown>).repeat_mode = rc.repeat_mode;
        }
        ...
        try {
          ...
          if (labels && labels.length > 0)
            await withRetry(() => client.tasks.updateTaskLabels(createdId, { label_ids: labels }), {
          ...
          if (assignees && assignees.length > 0) {
            try {
              await withRetry(
                () => client.tasks.bulkAssignUsersToTask(createdId, { user_ids: assignees }),
                {
                  maxRetries: RETRY_CONFIG.AUTH_ERRORS.maxRetries ?? 3,
                  timeout:
                    (RETRY_CONFIG.AUTH_ERRORS.initialDelay ?? 1000) +
          ...
          try {
            await client.tasks.deleteTask(createdId);
          } catch (deleteError) {
