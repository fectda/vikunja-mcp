...
### 4. Assignees ⚠️ BUG CONOCIDO

| Test | Input | Expected |
|------|-------|----------|
| Create task with assignees | `{ projectId: X, title: "Task", assignees: [userId] }` | `{ success: true, data: { assignees: [...] } }` DEBE guardar |
...
**BUG**: Assignees no persisten al crear tarea. Expected: `data.assignees` debe tener los users.
