...
| `/sdd-ff <name>`         | Fast-forward: proposal → tasks       |
...
- **Default**: `engram` (persistent memory)
- **File artifacts**: `openspec/changes/<change-name>/`
...
Run contract test to verify mocks are complete:

```bash
npm run test:contract
```

This detects missing mock methods automatically.
...
npm run test:contract  # Contract test (verify mocks are complete)
```
